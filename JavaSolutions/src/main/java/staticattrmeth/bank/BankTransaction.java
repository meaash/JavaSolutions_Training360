...
public class BankTransaction {
    ...
    private static long currentMinValue = MAX_TRX_VALUE; //a kezdő érték fordítva van megadva, hogy mindenképp kisebb
    ...
    public BankTransaction(long trxValue){
        ...
        //A BigDecimal immutable ezért uj változót kell neki létrehozni

        if (trxValue < currentMinValue) currentMinValue = trxValue; //mivel a static max van neki beállítva így nyilván kisebb lesz
    ...
    public static void initTheDay(){ //itt tudjuk kinullázni az eddigi számolásokat
        countTrx = 0;
    ...
    public static BigDecimal averageOfTransaction() {
        //BigDecimallal való számítások
        return countTrx == 0 ? new BigDecimal("0") :
                sumOfTrxs.divide(new BigDecimal(Long.toString(countTrx)), 0, RoundingMode.HALF_UP);
    }
